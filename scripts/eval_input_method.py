...
class Evaluator(object):
    ...
    def apply_ch(self, vi_ch, en_ch, smart=False):
        '''Applies an English letter to a single Vietnamese mark.
        ...
        Returns
        -------
        df : pandas.DataFrame(columns=['prefix', 'vi_prob', 'en_prob', 'vi_chosen', 'en_chosen'])
            dataframe of the same index as `self.df` but is a subset of `self.df`. The meanings are described in the followings. Column 'prefix' is the same as `self.df['word']` but with last character replaced by the English letter. Column 'vi_prob' is the same as `self.df['vi_prob']`. Column 'en_prob' represents the prior probability that the prefix is English, for each input prefix. Column 'vi_chosen' represents which prefix is chosen as 'vi'. Column 'en_chosen' represents which prefix is chosen as 'en'.
        '''
        df1 = self.dfs[vi_ch]

        word_s = df1['word'].str.slice(stop=-1) + en_ch # word prefixes
        ...
        return _pd.DataFrame(data={'prefix': word_s, 'vi_prob': vi_s, 'en_prob': en_s, 'vi_chosen': vi_chosen_s, 'en_chosen': en_chosen_s})
    ...
    def apply(self, input_method, smart=False):
        ...
        -------
        df : pandas.DataFrame(columns=['prefix', 'vi_prob', 'en_prob', 'vi_chosen', 'en_chosen'])
            dataframe of the same index as `self.df`. The meanings are described in the followings. Column 'prefix' is the same as `self.df['word']` but with last character replaced by the English letter. Column 'vi_prob' is the same as `self.df['vi_prob']`. Column 'en_prob' represents the prior probability that the prefix is English, for each input prefix. Column 'vi_chosen' represents which prefix is chosen as 'vi'. Column 'en_chosen' represents which prefix is chosen as 'en'.
        fer : float
            false english rate
        fvr : float
    ...
    def make_libunikey_prob_map_code(self, input_method, prefix, smart=False, prob_thresh=1E-6):
        df, fer, fvr = self.apply(input_method, smart=smart)

        with open("map_{prefix}.h".format(prefix=prefix), 'wt') as f:
            f.write("""// This file has been auto-generated by <envi_research_github_repository>/scripts/evaluator.py. Please do not modify but instead use the script to regenerate.

#ifndef MAP_{prefix}_H
#define MAP_{prefix}_H
...
extern std::unordered_map<std::string, std::pair<double,double>> map_{prefix};

#endif
""".format(prefix=prefix))

        with open("map_{prefix}.cpp".format(prefix=prefix), 'wt') as f:
            f.write("""// This file has been auto-generated by <envi_research_github_repository>/scripts/evaluator.py. Please do not modify but instead use the script to regenerate.
#include "map_{prefix}.h"


// map: prefix -> (english probability, vietnamese probability)
std::unordered_map<std::string, std::pair<double,double>> map_{prefix} = """.format(prefix=prefix))
            f.write("""{
  """)
            lines = []
            for _, row in df.iterrows():
                en_prob = row['en_prob']
                vi_prob = row['vi_prob']
                if en_prob < prob_thresh and vi_prob < prob_thresh:
                    continue
                lines.append('{"'+row['prefix']+'", {'+str(en_prob)+', '+str(vi_prob)+'}}')
            f.write(""",
  """.join(lines))
